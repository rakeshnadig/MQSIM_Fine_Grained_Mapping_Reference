...
int main(int argc, char* argv[])
{
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++)
	{
		...
		// FG - Printing useful information
		std::cout<<"DRAM size in megabytes : "<<ssd.dram_size<<std::endl;
		std::cout<<"Page size in kilobytes : "<<ssd.page_capacity<<std::endl;
		std::cout<<"Mapping unit in bytes : "<<ssd.mapping_unit<<std::endl;	
		std::cout<<"Sector size in bytes : "<<SECTOR_SIZE_IN_BYTE<<std::endl;
