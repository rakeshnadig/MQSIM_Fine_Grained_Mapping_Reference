...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	{
	public:
		AddressMappingDomain(unsigned int cmt_capacity, 
		...
		* proposal. It is always stored in non-volatile flash memory.*/
		...
		unsigned int mru_channel_id;
		unsigned int mru_chip_id;
		unsigned int mru_die_id;
		unsigned int mru_plane_id;
	...
	public:
		...
		//FGM - Functionality (3) - Methods to convert the physical address
		...
		//FGM - Functionality (4) - Setting a barrier for accessing a physical block during GC operations
	...
	private:
		...
		//FGM - Functionality (2) - The plane allocation algorithm in a Round-Robin fashion. 
		void allocate_new_plane_address(stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& targetAddress); //master method
		void calculate_resource_indices (
			...
			unsigned int* fourth_idx); //Round Robin 

		void allocate_plane_for_user_write(NVM_Transaction_Flash_WR* transaction); //slave methods
		...
		//FGM - Functionality (1) - Allocation of a page for a user/gc write transaction. 
