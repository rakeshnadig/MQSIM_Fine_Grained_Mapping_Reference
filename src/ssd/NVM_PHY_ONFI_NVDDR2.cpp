...
namespace SSD_Components {
	...
	NVM_PHY_ONFI_NVDDR2::NVM_PHY_ONFI_NVDDR2(const sim_object_id_type& id, 
		...
		: NVM_PHY_ONFI(id, 
				...
				channels(channels)
	{
		//FGM - To send mu_per_page to copy_read_data_to_transaction
		mu_per_page = mu_no_per_page;
	...
	//FGM - Called by AMU for reserve mapping, supporting multiple LPAs
	LPA_type NVM_PHY_ONFI_NVDDR2::Get_metadata(flash_channel_ID_type channe_id, flash_chip_ID_type chip_id, flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id, flash_mu_ID_type mu_id)//A simplification to decrease the complexity of GC execution! The GC unit may need to know the metadata of a page to decide if a page is valid or invalid. 
	{
		return channels[channe_id]->Chips[chip_id]->Get_metadata(die_id, plane_id, block_id, page_id, mu_id);
	}
	...
	void NVM_PHY_ONFI_NVDDR2::Send_command_to_chip(std::list<NVM_Transaction_Flash*>& transaction_list)
	{
		...
		* then something illegarl is happening*/
		...
		for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
			...
			//FGM - The transition from transactions to actual flash commands
			for (unsigned int mu_idx = 0; mu_idx< mu_per_page; mu_idx++)
			{
				metadata.LPAs.push_back(  (*it)->get_lpa(mu_idx) ); 
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				if (transaction_list.size() == 1) {
					...
					DEBUG("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending read command to chip for LPA: " << transaction_list.front()->get_lpa())
				} else {
					Stats::IssuedMultiplaneReadCMD++;
					dieBKE->ActiveCommand->CommandCode = CMD_READ_PAGE_MULTIPLANE;
					DEBUG("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending multi-plane read command to chip for LPA: " << transaction_list.front()->get_lpa())
				}
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					if (transaction_list.size() == 1) {
						...
						DEBUG("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending program command to chip for LPA: " << transaction_list.front()->get_lpa())
					} else {
						Stats::IssuedMultiplaneProgramCMD++;
						dieBKE->ActiveCommand->CommandCode = CMD_PROGRAM_PAGE_MULTIPLANE;
						DEBUG("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending multi-plane program command to chip for LPA: " << transaction_list.front()->get_lpa())
					}
	...
	void NVM_PHY_ONFI_NVDDR2:: copy_read_data_to_transaction(NVM_Transaction_Flash_RD* read_transaction, NVM::FlashMemory::Flash_Command* command)
	{
		...
		for (auto &address : command->Address) 
		{
			...
			{ 
				if (read_transaction->Source == Transaction_Source_Type::GC_WL)
				{
					for( unsigned int mu_idx =0; mu_idx< mu_per_page; mu_idx++)
					{
						read_transaction->set_waiting_lpas(command->Meta_data[mu_idx].LPAs[mu_idx]);
					}
				}
			}
		}
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
			...
			{
				//KL
				((NVM_Transaction_Flash_WR*)(*it))->Content = command->Meta_data[i].LPAs[i];
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		switch (bookKeepingEntry->ActiveTransactions.front()->Type)
		{
			case Transaction_Type::READ:
				...
				break;
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->get_related_read() == NULL) {
