...
namespace SSD_Components
{
	...
	{
		// FGM - Breaking the Logical Space Address based on the sectors/mapping unit
		LHA_type lsa = user_request->Start_LBA;
		unsigned int req_size = user_request->SizeInSectors;
		
		LHA_type starting_lsa = ((Input_Stream_NVMe*)input_streams[user_request->Stream_id])->Start_logical_sector_address ;
		LHA_type ending_lsa = ((Input_Stream_NVMe*)input_streams[user_request->Stream_id])->End_logical_sector_address;
		...
		{	
			...
			if (lsa < starting_lsa || lsa > ending_lsa) 
			{
				lsa = (starting_lsa + (lsa % ending_lsa - starting_lsa) );
			}
			//For each flow, all lsa's should be translated into a range starting from zero
			LHA_type internal_lsa = lsa - starting_lsa ;
		
			transaction_size = host_interface->sectors_per_mu - (unsigned int)(lsa % host_interface->sectors_per_mu);
			if (hanled_sectors_count + transaction_size >= req_size) 
			{
				transaction_size = req_size - hanled_sectors_count;
			}
			LPA_type lpa = internal_lsa / host_interface->sectors_per_mu;

			page_status_type temp = ~(0xffffffffffffffff << (int)transaction_size);
			access_status_bitmap = temp << (int)(internal_lsa % host_interface->sectors_per_mu);

			if (user_request->Type == UserRequestType::READ) {
				NVM_Transaction_Flash_RD* transaction = new NVM_Transaction_Flash_RD(Transaction_Source_Type::USERIO,
				...
				NVM_Transaction_Flash_WR* transaction = new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO, 
					user_request->Stream_id,
					transaction_size * SECTOR_SIZE_IN_BYTE, 
					lpa, 
					user_request, 
	...
	Host_Interface_NVMe::Host_Interface_NVMe(const sim_object_id_type& id,
		...
		Host_Interface_Base(id, HostInterface_Types::NVME, max_logical_sector_address, 
			sectors_per_page, 
			sectors_per_mu,
			cache),
