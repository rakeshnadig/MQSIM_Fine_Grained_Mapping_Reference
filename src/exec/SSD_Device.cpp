	...
	MQSimEngine::Sim_Object("SSDDevice")
{
	...
	//FG - Printing useful info in main:
	...
	//DRAM size in MB
	int log_dram_size = log2(parameters->Data_Cache_Capacity);
	dram_size = 1 << (log_dram_size - 20); //2^20 equals 1 MegaByte 
	//Page size in bytes
	page_capacity = parameters->Flash_Parameters.Page_Capacity;
	//Mapping unit in bytes
	mapping_unit = parameters->Mapping_Unit;


	//FG - Key variables
	sectors_per_mu = (parameters->Mapping_Unit / SECTOR_SIZE_IN_BYTE);
	mu_per_page =  (parameters->Flash_Parameters.Page_Capacity / parameters->Mapping_Unit) ;
		...
		case NVM::NVM_Type::FLASH: {
			...
			switch (parameters->Flash_Comm_Protocol) {
				case SSD_Components::ONFI_Protocol::NVDDR2: {
					...
					for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++) {
						...
						for (unsigned int chip_cntr = 0; chip_cntr < parameters->Chip_No_Per_Channel; chip_cntr++) {
							chips[chip_cntr] = new NVM::FlashMemory::Flash_Chip(device->ID() + ".Channel." + std::to_string(channel_cntr) + ".Chip." + std::to_string(chip_cntr),
								...
								read_latencies, 
								write_latencies, 
								parameters->Flash_Parameters.Block_Erase_Latency,
					...
					device->PHY = new SSD_Components::NVM_PHY_ONFI_NVDDR2(device->ID() + ".PHY", 
						channels, 
			...
			SSD_Components::FTL* ftl = new SSD_Components::FTL(device->ID() + ".FTL", 
				NULL, 
				parameters->Flash_Channel_Count,
				parameters->Chip_No_Per_Channel, 
				parameters->Flash_Parameters.Die_No_Per_Chip, 
			...
			switch (parameters->Address_Mapping) {
				case SSD_Components::Flash_Address_Mapping_Type::PAGE_LEVEL:
					amu = new SSD_Components::Address_Mapping_Unit_Page_Level(ftl->ID() + ".AddressMappingUnit", 
						ftl, (SSD_Components::NVM_PHY_ONFI*) device->PHY,
						fbm, 
						parameters->Ideal_Mapping_Table, 
			...
			gcwl = new SSD_Components::GC_and_WL_Unit_Page_Level(ftl->ID() + ".GCandWLUnit", 
				amu, 
				fbm, 
				tsu,  
				(SSD_Components::NVM_PHY_ONFI*)device->PHY,
			...
			switch (parameters->Caching_Mechanism) {
				case SSD_Components::Caching_Mechanism::SIMPLE:
					dcm = new SSD_Components::Data_Cache_Manager_Flash_Simple(device->ID() + ".DataCache", NULL, 
						ftl, 
						(SSD_Components::NVM_PHY_ONFI*) device->PHY,
				...
				case SSD_Components::Caching_Mechanism::ADVANCED:
					dcm = new SSD_Components::Data_Cache_Manager_Flash_Advanced(device->ID() + ".DataCache", NULL, 
						ftl, 
						(SSD_Components::NVM_PHY_ONFI*) device->PHY,
						parameters->Data_Cache_Capacity, 
						parameters->Data_Cache_DRAM_Row_Size, 
			...
			//FG - Since MQSim considers dcm a non FTL component, we connect dcm with gcwl, to support merged GC transactions
			gcwl->dcm = dcm;
