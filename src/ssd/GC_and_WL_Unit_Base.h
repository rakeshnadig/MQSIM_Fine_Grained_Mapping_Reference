	...
	*/
	...
	public:
		GC_and_WL_Unit_Base(const sim_object_id_type& id, 
			...
			TSU_Base* tsu, NVM_PHY_ONFI* flash_controller,
		...
		//FGM - Connecting GCWL with a dcm object
	...
	protected:
		...
		std::vector<LPA_type> buffered_lpas ;
		std::vector<page_status_type> buffered_page_status;
		std::vector<NVM::FlashMemory::Physical_Page_Address> buffered_addresses;
		std::vector<stream_id_type> buffered_streams;
	};
}
