				...
				if (block->Current_page_write_index - block->Invalid_page_count > 0) {
					...
					for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) 
					{
						if (block_manager->Is_page_valid(block, pageID)) 
						{
							...
							gc_candidate_address.MUID = 0;
							if (use_copyback) 
							{
								gc_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block->Stream_id, sectors_per_mu* SECTOR_SIZE_IN_BYTE,
							...
							{
								//FGM - Iterate through each valid mapping unit of the page
								bool flag = false;
								for (unsigned int mu_idx=0; mu_idx<mu_per_page; mu_idx++)
								{
									if (block_manager->Is_mu_valid(block, pageID, mu_idx))
									{ 
										//If there is at least one valid mapping unit, create a new GC RD transaction.
										//If there is a valid mapping unit from the same page, update the GC RD transaction. 
										if (!flag)
										{
											gc_candidate_address.MUID = mu_idx;	
								...
								gc_read->RelatedErase = gc_erase_tr;
