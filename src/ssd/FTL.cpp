...
namespace SSD_Components
{
	...
	void FTL::Perform_precondition(std::vector<Utils::Workload_Statistics*> workload_stats)
	{
		...
		for (auto &stat : workload_stats)
		{
			...
			total_accessed_cmt_entries += (unsigned int)(Convert_host_logical_address_to_device_address(max_lha) / sectors_per_mu - Convert_host_logical_address_to_device_address(min_lha) / sectors_per_mu) + 1;
			...
			if (stat->Type == Utils::Workload_Type::SYNTHETIC)
			{
				...
				while (lpa_set_for_preconditioning.size() < no_of_logical_pages_in_steadystate)
				{
					...
					{
						transaction_size = sectors_per_mu - (unsigned int)(lsa % sectors_per_mu);
				...
				while (lpa_set_for_preconditioning.size() < no_of_logical_pages_in_steadystate) {
					...
					while (hanled_sectors_count < size) {
						...
						transaction_size = sectors_per_mu - (unsigned int)(internal_lsa % sectors_per_mu);
						if (hanled_sectors_count + transaction_size >= size) {
							transaction_size = size - hanled_sectors_count;
						}
	...
	{
		return lha / sectors_per_mu;
	}

	page_status_type FTL::Find_NVM_subunit_access_bitmap(LHA_type lha)
	{
		return ((page_status_type)~(0xffffffffffffffff << (int)1)) << (int)(lha % sectors_per_mu);
	}
}
