...
namespace SSD_Components
{
	...
	{
		// this method is called for each block that has an x amount of valid pages 
	...
	{
		//FGM - This method is called by AMU. Since AMU iterates through each mapping unit, this mehtod is called for all mapping units

		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		Block_Pool_Slot_Type* block = &plane_record->Blocks[page_address.BlockID];
	
		//Invalidating this mapping unit
		block->Invalid_mu_count++;
		block->Valid_page[page_address.PageID][page_address.MUID] = false; 

		//Checking if the page is invalid
		if (!Is_page_valid(block, page_address.PageID))
		{
	...
	inline void Flash_Block_Manager::Invalidate_page_in_block_for_preconditioning(const stream_id_type stream_id, const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		{
			block-> Valid_page[page_address.PageID][mu_idx] = false; 
		}

		//Checking if the page is invalid
		if (Is_page_valid(block, page_address.PageID)) 
			PRINT_ERROR("You did not invalidated the page");
	}
