...
namespace NVM
{
	namespace FlashMemory
	{
		...
		void Flash_Chip::Change_memory_status_preconditioning(const NVM_Memory_Address* address, const void* status_info)
		{
			...
			Dies[flash_address->DieID]->Planes[flash_address->PlaneID]->Blocks[flash_address->BlockID]->Pages[flash_address->PageID].Metadata.LPAs.push_back( *(LPA_type*)status_info );	
		}
		...
		//FGM - Called by AMU for reverse mapping, i.e. map the LPAs while bringing them from the back end 
		LPA_type Flash_Chip::Get_metadata(flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id, flash_mu_ID_type mu_id)//A simplification to decrease the complexity of GC execution! The GC unit may need to know the metadata of a page to decide if a page is valid or invalid. 
		{
			Page* page = &(Dies[die_id]->Planes[plane_id]->Blocks[block_id]->Pages[page_id]);
			return Dies[die_id]->Planes[plane_id]->Blocks[block_id]->Pages[page_id].return_metadata(mu_id);
			...
			switch (command->CommandCode)
			{
				...
				case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						//FGM - Metadata size = mapping unit/page
						for (unsigned int mu_idx =0; mu_idx< command->Meta_data.size(); mu_idx++)
						{
							targetDie->Planes[command->Address[planeCntr].PlaneID]->Blocks[command->Address[planeCntr].BlockID]->Pages[command->Address[planeCntr].PageID].get_metadata(command->Meta_data[mu_idx], mu_idx);
						} 
					}
					break;
				...
				case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						for (unsigned int mu_idx =0; mu_idx< command->Meta_data.size(); mu_idx++)
						{
							targetDie->Planes[command->Address[planeCntr].PlaneID]->Blocks[command->Address[planeCntr].BlockID]->Pages[command->Address[planeCntr].PageID].set_metadata(command->Meta_data[mu_idx], mu_idx);
						} 
				...
				case CMD_ERASE_BLOCK_MULTIPLANE:
				{
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						for (unsigned int i = 0; i < page_no_per_block; i++) {
							...
							//KL
							targetBlock->Pages[i].Metadata.LPAs.push_back ( NO_LPA );
						}
					}
					break;
				}
