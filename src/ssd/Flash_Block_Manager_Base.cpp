...
namespace SSD_Components
{
	...
	unsigned int Block_Pool_Slot_Type::Mu_vector_size = 0;
				...
				for (unsigned int dieID = 0; dieID < die_no_per_chip; dieID++) {
					...
					for (unsigned int planeID = 0; planeID < plane_no_per_die; planeID++) {
						...
						for (unsigned int blockID = 0; blockID < block_no_per_plane; blockID++) {
							...
							//FGM - Initialization of the new mechanism 
							Block_Pool_Slot_Type::Mu_vector_size = mu_per_page / (sizeof(uint8_t) * 8) + (mu_per_page % (sizeof(uint8_t) * 8) == 0 ? 0 : 1);						plane_manager[channelID][chipID][dieID][planeID].Add_to_free_block_pool(&plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID], false);
							plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID].Valid_page = new bool*[pages_no_per_block];
							for (unsigned int pageID = 0; pageID < pages_no_per_block; pageID++)
							{	
								plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID].Valid_page[pageID] = new bool [mu_per_page];
								for (unsigned int mu_idx=0; mu_idx<mu_per_page; mu_idx++)
								{	
									plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID].Valid_page[pageID][mu_idx] = true;
								}
							}
				...
				for (unsigned int die_id = 0; die_id < die_no_per_chip; die_id++) {
					for (unsigned int plane_id = 0; plane_id < plane_no_per_die; plane_id++) {
						for (unsigned int blockID = 0; blockID < block_no_per_plane; blockID++) {
							...
							for (unsigned int pageID = 0; pageID < pages_no_per_block ; pageID ++)
							{
								delete[] plane_manager[channel_id][chip_id][die_id][plane_id].Blocks[blockID].Valid_page[pageID]; 
							}
							delete[] plane_manager[channel_id][chip_id][die_id][plane_id].Blocks[blockID].Valid_page;
						}
	...
	bool Flash_Block_Manager_Base::Is_page_valid(Block_Pool_Slot_Type* block, flash_page_ID_type page_id)
	{
		...
		for (unsigned int mu_idx=0; mu_idx<mu_per_page; mu_idx++)
		{
			//FGM - To consider a page valid, there should be at least one valid mapping unit.
			if (block->Valid_page[page_id][mu_idx]) flag = true; 
		}
		if (flag) return true;
		else return false;
	} 
	//FGM
	bool Flash_Block_Manager_Base ::Is_mu_valid( Block_Pool_Slot_Type* block, flash_page_ID_type page_id, flash_mu_ID_type mu_idx) 
	{
		if (block->Valid_page[page_id][mu_idx]) return true;
		else return false; 
	}
}
